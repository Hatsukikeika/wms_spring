...
public class AccountServiceImpl implements AccountService {
	...
	public void createRootAccount(Map<String, Object> signup) {
		...
		if(gtype == GroupType.TYPE_SELLER || gtype == GroupType.TYPE_STORAGE) {
			// Parameter is save
			// create user group
			Company company = new Company().setType(gtype)
					.setActivated(false)
					.setName((String) signup.get("cname"));
			...
			User user = new User(company).setActivated(false).setEmail(email).setPassword(bCryptPasswordEncoder.encode(password));
	...
	private void sendValidationEmail(String email, Long compid, String tempass, Validation val) {
		try {
			...
			if(tempass != null) {
				...
				subject = String.format("An invitaion from %s %s", company.getOwner().getFirstname(), company.getOwner().getLastname());
			}
			...
			new AccMailContent()
					.addReceivers(email)
					.setSubject(subject)
					.setTextBody("Please remember your company name to login <br/>" + "Company Name: " + company.getName() + "<br/>"
